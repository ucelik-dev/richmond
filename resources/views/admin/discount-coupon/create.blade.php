@extends('admin.layouts.master')

@section('content')
    <div class="page-body">
        <div class="container-xl">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">DISCOUNT COUPON CREATE</h3>
                    <div class="card-actions">
                        <a href="{{ route('admin.discount-coupon.index') }}" class="btn btn-dark px-2 py-1 px-md-3 py-md-2">
                            <i class="fa-solid fa-arrow-left me-2"></i>
                            Back
                        </a>
                    </div>
                </div>

                <div class="card-body">
                    <div class="add_course_basic_info">
                        <form action="{{ route('admin.discount-coupon.store') }}" method="POST">
                            @csrf

                            <div class="row">
                                <div class="col-xl-4">
                                    <div class="general_form_input">
                                        <label class="label-required">Code</label>
                                        <div class="input-group">
                                            <input type="text" name="code" class="form-control" value="{{ old('code') }}" placeholder="">
                                            <button type="button" id="btnGenCode" class="btn btn-dark py-1">Generate</button>
                                        </div>
                                    </div>
                                </div>

                                <div class="col-xl-4">
                                    <div class="general_form_input">
                                        <label class="label-required">Agent</label>
                                        <select name="agent_id" class="form-control form-select" required>
                                            <option value="">Select an agent</option>
                                            @foreach($agents as $agent)
                                                <option value="{{ $agent->id }}" @selected(old('agent_id')==$agent->id)>
                                                    {{ $agent->company }} - {{ $agent->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>

                                <div class="col-xl-4">
                                    <div class="general_form_input">
                                        <label class="label-required">Discount Type</label>
                                        <select name="discount_type" class="form-control form-select" required>
                                            <option value="percent" @selected(old('discount_type','percent')==='percent')>Percent (%)</option>
                                            <option value="fixed"   @selected(old('discount_type')==='fixed')>Fixed amount</option>
                                        </select>
                                    </div>
                                </div>

                                <div class="col-xl-4">
                                    <div class="general_form_input">
                                        <label class="label-required">Discount Value</label>
                                        <input type="number" step="0.01" min="0.01" name="discount_value" class="form-control"
                                               value="{{ old('discount_value') }}" placeholder="e.g. 10 or 99.99" required>
                                        <small class="text-secondary">If type is percent, max 100.</small>
                                    </div>
                                </div>

                                <div class="col-xl-4">
                                    <div class="general_form_input">
                                        <label class="">Minimum Amount</label>
                                        <input type="number" step="0.01" min="0" name="min_amount" class="form-control"
                                               value="{{ old('min_amount') }}" placeholder="Optional">
                                    </div>
                                </div>

                                <div class="col-xl-4">
                                    <div class="general_form_input">
                                        <label class="">Max Uses</label>
                                        <input type="number" step="1" min="1" name="max_uses" class="form-control"
                                               value="{{ old('max_uses') }}" placeholder="Leave empty for unlimited">
                                    </div>
                                </div>

                                <div class="col-xl-4">
                                    <div class="general_form_input">
                                        <label class="label-required">Status</label>
                                        @php $statusOld = (string) old('status', '1'); @endphp
                                        <select name="status" class="form-control form-select" required>
                                            <option value="1">Active</option>
                                            <option value="0">Inactive</option>
                                        </select>
                                    </div>
                                </div>

                            </div>

                            <button type="submit" class="btn btn-dark px-2 py-1 px-md-3 py-md-2 mt-2">Create</button>
                        </form>
                    </div>
                </div>

            </div>
        </div>
    </div>
@endsection

@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const btn  = document.getElementById('btnGenCode');
            const code = document.querySelector('input[name="code"]');
            if (!btn || !code) return;
            btn.addEventListener('click', function () {
                fetch('{{ route('admin.discount-coupon.generate-code') }}')
                .then(r => r.json())
                .then(({code: c}) => { code.value = c; })
                .catch(() => alert('Could not generate code.'));
            });
        });
    </script>
@endpush